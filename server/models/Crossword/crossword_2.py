...
def stats(cwd, print_missing=True):
    ...
    print(
        "total-cells/empty-cells- Quotient: %.1f/1 (%i, %i)" % (round(num_cells / num_empty, 1), num_cells, num_empty))
...
def run_benchmark_test(words=100, num=100, bestof=3):
    ...
    print
    formatter.get_crossword_ascii_grid(solved=True, printable=True)
...
class CrossWordFormatter(object):
    ...
    def __init__(self, crossword, ppb=32, solution=None, transparency=False, order=True):
        ...
        self.highlight_colors = [(80, 73, 158, 128), (75, 191, 84, 128), (219, 61, 101, 128), (237, 230, 36, 128),
                                 (132, 215, 240, 128)]
    ...
    def _set_solution(self, solution):
        ...
        for letter in solution:
            ...
            if solution.count(letter) > len(self.crossword.letters[letter]):
                raise SolutionError(
                    "Your solution '%(solution)s' has %(num)i '%(letter)s' - there are not that much '%(letter)s's in your crosssword!" % {
    ...
    def get_crossword_ascii_grid(self, solved=False, printable=False):
        ...
        if not solved: #replace answer with _ spaces
            outStr = re.sub(r'[a-z]', '_', outStr)
        return outStr.replace(" ","x")
        #return outStr
    ...
    def get_crossword_image_grid(self, output, solved=False):
        ...
        for letter in self.crossword.letters:
            for col, row in self.crossword.letters[letter]:
                col -= 1
                row -= 1
                draw.polygon([col * ppb, row * ppb, col * ppb + shadow_depth, row * ppb - shadow_depth,
                              (col + 1) * ppb + shadow_depth, row * ppb - shadow_depth, (col + 1) * ppb + shadow_depth,
                              (row + 1) * ppb - shadow_depth, (col + 1) * ppb, (row + 1) * ppb, col * ppb,
                              (row + 1) * ppb, col * ppb, row * ppb], fill=self.colors["shadow-grid"],
                             outline=self.colors["shadow-grid"])
        ...
        if self.solution:
            for col, row in solution_coords:
                if col and row:
                    ...
                    row -= 1
                    draw.polygon([col * ppb, row * ppb, col * ppb + shadow_depth, row * ppb - shadow_depth,
                                  (col + 1) * ppb + shadow_depth, row * ppb - shadow_depth,
                                  (col + 1) * ppb + shadow_depth, (row + 1) * ppb - shadow_depth, (col + 1) * ppb,
                                  (row + 1) * ppb, col * ppb, (row + 1) * ppb, col * ppb, row * ppb],
                                 fill=self.colors["shadow-grid"], outline=self.colors["shadow-grid"])
        ...
        for col in range(self.crossword.cols):
            ...
            for cell in self.crossword.grid[col]:
                if not cell == self.crossword.empty:
                    ...
                    draw.rectangle([col * self.ppb, row * self.ppb, (col + 1) * self.ppb, (row + 1) * self.ppb],
                                   outline=self.colors["grid"], fill=fill_color)
                row += 1
        if self.solution:
            highlight_color = 0
            for col, row in solution_coords:
        ...
        for word in self.crossword.placed_words:
            ...
            if solved:
                ...
                for letter in word.word:
                    ...
                    if word.vertical:
                        draw.text(((col - 1) * self.ppb + letter_offset_x,
                                   (row - 1 + letter_counter) * self.ppb + letter_offset_y), str(letter), fill="black",
                                  font=font)
                    else:
                        draw.text(((col - 1 + letter_counter) * self.ppb + letter_offset_x,
                                   (row - 1) * self.ppb + letter_offset_y), str(letter), fill="black", font=font)
                    letter_counter += 1
            ...
            if (col, row) in self.blocked_fields:
                ...
                y_offset = 0
            draw.text(((col - 1) * self.ppb + num_offset, (row - 1) * self.ppb + num_offset + y_offset),
                      str(word.number), fill=self.colors["text"], font=number_font)
            w, h = draw.textsize(str(word.number), font=number_font)
            # ~ w = w/len(str(word.number))
            if word.vertical:
                self._draw_arrow(draw, "down", (col - 1) * self.ppb + num_offset + w,
                                 (row - 1) * self.ppb + num_offset + y_offset, w / len(str(word.number)), h)
            else:
                self._draw_arrow(draw, "right", (col - 1) * self.ppb + num_offset + w,
                                 (row - 1) * self.ppb + num_offset + y_offset, w / len(str(word.number)), h)
        ...
        if solved and self.solution:
            for letter in self.solution[::-1]:
                ...
                if col and row:
                    draw.text(((col - 1) * self.ppb + letter_offset_x, (row - 1) * self.ppb + letter_offset_y), letter,
...
def multiple_crosswords(cols, rows, empty="-", maxloops=2000, wordlist=[], num=10, time_permitted=2.0, best_of=10,
                        force_solved=False):
...
class CrossWord(object):
    ...
    def compute_crossword(self, rounds=2, best_of=3, force_solved=False):
        ...
        while (count <= best_of - 1 and not force_solved) or (force_solved and not solved):
            ...
            if (len(copy.placed_words) >= len(self.placed_words) and score >= best_score) or len(
                    copy.placed_words) > len(self.placed_words):
...
def run( print_cross):
    ...
    for inputfile in args:
        ...
        if missing != []:
            print(
                "Could not place some words. Probably your grid is too small. Sometimes setting \"--bestof\" to a higer value also help.")
